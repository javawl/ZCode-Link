...
import linkAgentIconUrl from "@/assets/brand/linkagent-icon.png";
...
export function ConversationDraftEmptyState({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      <div
        aria-hidden="true"
        className="pointer-events-none absolute bottom-full left-1/2 mb-6 size-16 -translate-x-1/2 sm:size-20"
      >
        <ZCodeEmptyStateLogo className="h-full w-full object-contain" />
      </div>
      <p
...
function ZCodeEmptyStateLogo({ className }: { className?: string }) {
  return (
    <img
      aria-hidden="true"
      className={className}
      data-v4-draft-logo="linkagent"
      src={linkAgentIconUrl}
      alt=""
    />
  );
}
